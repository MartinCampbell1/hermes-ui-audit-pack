...
def _list_recent_sessions(db, limit: int, current_session_id: str = None) -> str:
    ...
    try:
        sessions = db.list_threads_rich(limit=limit + 5, exclude_sources=list(_HIDDEN_SESSION_SOURCES))  # fetch extra to skip current
        ...
        for s in sessions:
            ...
            if current_root and (
                sid == current_root
                or sid == current_session_id
                or s.get("thread_id") == current_root
            ):
                continue
            results.append({
                "session_id": sid,
